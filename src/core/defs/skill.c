...
static void skill_init(struct skill *skill,const char *name, 
                     const char *description, enum skill_target_class target_class,
                     struct unit_stats req_stats, struct skill *req_learned, 
...
void skills_load(struct skill all_skills[SKILL_COUNT])
{
    ...
    skill_init(&all_skills[SKILL_SKIP_TURN], 
    ...
    skill_init(&all_skills[SKILL_USUAL_SLASH], 
    ...
    skill_init(&all_skills[SKILL_USUAL_STABBING], 
               "Usual stabbing", "Base damage",
               SKILL_TARGET_ENEMY_UNIT, req_stats, ROOT_SKILL, 
               8, NATURE_WEAPON_GROUP, (uint64_t)1<<UNIT_TAG_HUMAN);

    unit_stats_init(&req_stats, 5, 5, 0, 0);
    skill_init(&all_skills[SKILL_MAUL], 
               "Maul", "Maul",
               SKILL_TARGET_ENEMY_UNIT, req_stats, ROOT_SKILL, 
